namespace SG_Tool.Log
{
    public class SystemLog_Form : Form
    {
        ...
        static string m_strLogPath = GetLogFilePath(string.Empty);
        ...
        /// </summary>
        static string GetLogFilePath(string strLogName)
        {
            if (strLogName == string.Empty)
            {
            ...
            {
                string todayFileName = $"{strLogName}_{DateTime.Now:yyyyMMdd}_log.txt";
                return Path.Combine(LogDirectory, todayFileName);
            }
        }
        ...
        static void SaveBufferedLogs()
        {
            lock (logLock)
            {
                ...
                {
                    EnsureLogDirectory();
                    m_strLogPath = GetLogFilePath(string.Empty);
        ...
        public static void LogMessage(TextBox txtLog, string message, int nType = 0, bool overwrite = false)
        {
            try
            {
                ...
                // 즉시 저장
                SaveImmediate(logMessage, txtLog.Name);
            }
            catch (Exception ex)
            {
                lock (logLock)
                {
                    ...
                    SaveImmediate(m_strlogMessage, "Common");
                }
            }
        }
        ...
        /// 즉시 로그 저장
        /// </summary>
        static void SaveImmediate(string strLogMessage, string strLogName)
        {
            try
            {
                EnsureLogDirectory();
                m_strLogPath = GetLogFilePath(strLogName);
                File.AppendAllText(m_strLogPath, strLogMessage);
            }
            catch
            {
